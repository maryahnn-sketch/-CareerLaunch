...
8) Discovery balance: include at least one Strong or Moderate direct-fit path even when it feels familiar. Also include at least one adjacent or less-obvious path when retained skills support it. Do not return only paths the user already named. Do not force random novelty, exaggerated titles, unsupported salary claims, or fabricated qualifications.`,

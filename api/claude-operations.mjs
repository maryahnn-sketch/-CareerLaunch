...
export const MAX_EXTRA_REPAIR_HINT_CHARS = 2_000;
...
export const OPERATION_REGISTRY = {
  analyzeSkills: { mode: 'structured', toolName: 'report_skills' },
  discoverPaths: { mode: 'structured', toolName: 'report_career_paths' },
  sendConvo: { mode: 'plain', toolName: null },
  sendConvoProfileDelta: { mode: 'structured', toolName: 'report_profile_delta' },
  refinePaths: { mode: 'structured', toolName: 'report_refined_career_paths' },
  rerankPaths: { mode: 'structured', toolName: 'report_reranked_paths' },
  buildRoadmapFoundation: { mode: 'structured', toolName: 'report_roadmap_foundation' },
  buildRoadmapActionPlan: { mode: 'structured', toolName: 'report_roadmap_action_plan' },
  buildRoadmapDirection: { mode: 'structured', toolName: 'report_roadmap_direction' },
  buildKit: { mode: 'structured', toolName: 'report_application_kit' },
  strengthenBullet: { mode: 'structured', toolName: 'report_rewritten_bullet' },
  submitAddExperience: { mode: 'structured', toolName: 'report_skills' },
  buildStoryBank: { mode: 'structured', toolName: 'report_story_bank' },
  addStoryDetail: { mode: 'structured', toolName: 'report_story' },
  analyzeJd: { mode: 'structured', toolName: 'report_jd_analysis' },
};

function rejectedSkillsClause(rejectedSkillNames) {
  if (!Array.isArray(rejectedSkillNames) || !rejectedSkillNames.length) return '';
  return `\nThe user explicitly reviewed and REJECTED these as inaccurate: ${rejectedSkillNames.join(', ')}. Even though the story text below may still describe the underlying activity, treat these as struck from the record: do not name them, imply them, or use their evidence as a transfer, reason, or fit signal anywhere in your response.`;
}
...
7) title = the entryPoint; why max 18 words, evidence-based, no filler; transfers = 2-3 exact names from the validated skills list; gaps = 1-2 short phrases (max 5 words); transition is REQUIRED on every single path and must be exactly one of "Strong" | "Moderate" | "Developing" | "Needs More Information" — never omit it, and never leave it blank. fitEvidence is optional and, if included, uses the same four values. Do NOT generate interest or lifestyle fit — the user hasn't given preference data yet, so those are filled in later (after the results conversation and reranking), not here. Keep every field concise.${rejectedSkillsClause(rejected)}`,
...
${NO_INVENTION_RULE}
${VERIFIED_INFO_RULE}
Reply in 2-4 sentences of plain text only — no markdown, no JSON, no preamble, no field labels.${rejectedSkillsClause(rejected)}`,

  sendConvoProfileDelta: (rejected) =>
    `You extract structured preference signals for iFindWorth's Career Intelligence Profile from one turn of a career-discovery conversation. Only include a genuine new preference signal — something the user wants more or less of in their work — never invent one. If nothing new was signaled, return empty arrays and an empty summary. Never repeat a preference already known.${rejectedSkillsClause(rejected)}`,

  refinePaths: (rejected) =>
...
8) User interest is NOT evidence.${rejectedSkillsClause(rejected)}`,

  rerankPaths: (rejected) =>
...
6) changeSummary (max 25 words) names the preferences that drove the reranking.${rejectedSkillsClause(rejected)}`,

  buildRoadmapFoundation: (rejected) =>
...
4) Only draw on the "Validated skills" list below — the user accepted these for use in their analysis (and rejected any marked "Not quite"). Do not incorporate any other skill or activity the story text might mention.${rejectedSkillsClause(rejected)}`,

  strengthenBullet: (rejected) =>
    `You rewrite a single resume bullet to incorporate one new true detail the user just provided. ${NO_INVENTION_RULE} ${VERIFIED_INFO_RULE} Only use the exact detail given — do not round, estimate, or embellish beyond it. One line, action-verb led.${rejectedSkillsClause(rejected)}`,
...
${NO_INVENTION_RULE}
${VERIFIED_INFO_RULE}
Rules: marketAsks = 4-6 short requirements/keywords pulled from the posting text itself. youAlreadyHave = ONLY skill names copied exactly from the "User's validated skills" list below that genuinely match something in the posting — never a rejected skill, never one inferred from outside the posting. shouldStrengthen = 2-3 gaps between the posting and the user's validated skills. overallFit = one of "Strong Match","Worth Considering","Stretch Opportunity","Needs More Information".${rejectedSkillsClause(rejected)}`,
};

function buildReshapeSystem(context) {
  const failureDetail = context.reshapeFailureDetail || 'schema validation failed';
  let system = `You are a strict data formatter for iFindWorth. You will be given content that was already generated but does not match the required schema exactly (reason it failed: ${failureDetail}). Reshape it to match the schema EXACTLY — fix field names, enum wording/casing, and string-vs-array mismatches, and restructure as needed. Use ONLY the substance already present in the given content. Do NOT invent new career paths, skills, reasoning, gaps, or any other fact. Do NOT change the meaning of any field — only its format.`;
  if (context.extraRepairHint) system += ` ${context.extraRepairHint}`;
  return system;
}
...
function normalizeContext(rawContext, suffix) {
  ...
  if (suffix === 'reshape') {
    ...
    allowedKeys.add('extraRepairHint');
  }
  ...
  for (const name of rejectedSkillNames) {
    if (typeof name !== 'string' || !name.trim() || name.length > MAX_REJECTED_SKILL_NAME_CHARS) {
  ...
  if (suffix === 'reshape') {
    ...
    if (rawContext.extraRepairHint !== undefined && rawContext.extraRepairHint !== null) {
      if (typeof rawContext.extraRepairHint !== 'string') {
        return { ok: false, error: 'context.extraRepairHint must be a string' };
      }
      if (rawContext.extraRepairHint.length > MAX_EXTRA_REPAIR_HINT_CHARS) {
        return { ok: false, error: 'context.extraRepairHint exceeds allowed length', status: 413 };
      }
      context.extraRepairHint = rawContext.extraRepairHint.trim();
    }
  } else if (
    rawContext.reshapeFailureDetail !== undefined ||
    rawContext.extraRepairHint !== undefined
  ) {
...
function buildSystemPrompt(baseOperation, suffix, context) {
  if (suffix === 'reshape') {
    return buildReshapeSystem(context);
  }

  const builder = SYSTEM_BUILDERS[baseOperation];
  if (!builder) return null;
  return builder(context.rejectedSkillNames);
}
...
export function prepareAnthropicRequest(body) {
  ...
  const system = buildSystemPrompt(base, suffix, contextCheck.context);
  if (!system) {
    return { ok: false, error: 'unsupported operation' };
  }
